...
export async function removeContact(contactId: string, companyId: string): Promise<ActionResult> {
  const supabase = await createClient();
...
export async function attachContact(contactId: string, companyId: string): Promise<ActionResult> {
  const supabase = await createClient();
